...
- (void)EnemyMove:(float)EnemyX:(int)score
{
    ...
    NSLog(@"EnemyMoveのscoreの値:%d", score);
    
    // アニメーションのオプションを設定
    animation.duration = 0.5-speed/10; // アニメーション速度
    animation.repeatCount = HUGE_VALF; // 繰り返し回数
...
{
    // 始点と終点を設定
    NSLog(@"changeAnimationのscoreの値:%.d", score);
    
    // score->敵の撃破数
    // 5体倒す毎にアニメーションスピード(敵の動くスピード)を速くする。
    if(score % 5 == 0){
        speed -= 0.2;
    }
    animation.duration = speed; // アニメーション速度
